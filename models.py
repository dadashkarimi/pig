...
def labels_to_image_new(
    ...
    Parameters:
        ...
        up_scale: Symmetric scaling or just downsampling
...
def labels_to_windows_new(
    ...
    return_bias=False,
    id=0,
    positions=[],  # Added positions parameter
    window_size=12  # Added window_size parameter
):
    """Build model that augments label maps and synthesizes images from them.
    ...
    if return_im and return_map:
        windows, window_labels = extract_windows_and_labels(image, labels, window_size, positions)
        print("ddd",len(windows))
        outputs.append(windows)
        outputs.append(window_labels)
        assert not seeds, f'unknown seeds {seeds}'
        return tf.keras.Model(input_model.inputs, outputs)
...
def extract_windows_and_labels(image, label, window_size, positions):
    """
    Extract 3D windows from the image and label, and determine the label for each window.

    Parameters:
    - image: The input image tensor of shape (1, 192, 192, 192, 1).
    - label: The label tensor of shape (1, 192, 192, 192, 1).
    - window_size: The size of the window to extract (a scalar, assuming cubic windows).
    - positions: A list of tuples representing the positions of the windows to extract.

    Returns:
    - windows: A list of 3D windows extracted from the image.
    - labels: A list of labels for each window (1 if the window contains non-zero elements, otherwise 0).
    """
    windows = []
    labels = []
    ...
    for pos in positions:
        ...
        window = image[0, x1:x2, y1:y2, z1:z2, 0]
        window_label = label[0, x1:x2, y1:y2, z1:z2, 0]

        windows.append(window)
        labels.append(1 if tf.reduce_sum(window_label) > 0 else 0)
