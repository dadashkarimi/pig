...
def resize_with_surfa(X):
    # X_np = X.asarray()
    # scale_factors = [float(target_dim[i]) / X.shape[i] for i in range(len(X.shape))]
    # resized_np = sf.Volume(X_np).resize(scale_factors).asarray()
    # resized_np_reshaped = resized_np.reshape((target_dim[0], target_dim[1], target_dim[2], -1))

    resized_np = sf.Volume(X_np).resize(scale_factors)
    resized_np.data = np.clip(resized_np.data, 0, 255)
    # resized_np_uint8 = resized_np.data.astype(np.uint8)
    # resized_np_reshaped = resized_np.reshape((target_dim[0], target_dim[1], target_dim[2], -1))

    return resized_np.data
...
def sliding_window(img, thr, net, input_12_node):
    ...
    for scale in xrange(param.pyramid_num):
        ...
        resized = resize_with_surfa(X)#, target_dim=(param.img_size_12, param.img_size_12, param.input_channel))
