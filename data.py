...
def crop_2d_image(image, xmin, ymin, xmax, ymax):
    """
    Crop a 2D image using native Python.

    Args:
    - image (list): The 2D image represented as a list of lists.
    - xmin, ymin, xmax, ymax (int): Coordinates of the bounding box.

    Returns:
    - cropped_image (list): The cropped image.
    """
    cropped_image = []
    for row in range(ymin, ymax + 1):
        cropped_image_row = image[row][xmin:xmax + 1]
        cropped_image.append(cropped_image_row)
    return cropped_image

def crop_3d_volume(volume, xmin, ymin, zmin, xmax, ymax, zmax):
    cropped_volume = []
    for depth in range(zmin, zmax + 1):
        cropped_image = crop_2d_image(volume[:, :, depth], xmin, ymin, xmax, ymax)
        cropped_volume.append(cropped_image)
    return np.stack(cropped_volume, axis=-1)
...
def flip_left_right_volume(volume):
    flipped_volume = [flip_left_right(image) for image in volume]
    ...
    #     rad_rand = random.randint(0, min(x_shape[1], x_shape[2]) - 1)
    #     while rad_rand <= param.face_minimum:
    #         rad_rand = random.randint(0, min(x_shape[1], x_shape[2]) - 1)

    #     x_rand = random.randint(0, x_shape[2] - rad_rand - 1)
    #     y_rand = random.randint(0, x_shape[1] - rad_rand - 1)
    #     z_rand = random.randint(0, x_shape[0] - 1)

    #     neg_cropped_img = x[z_rand, y_rand:y_rand + rad_rand, x_rand:x_rand + rad_rand]
    #     neg_cropped_img = Image.fromarray(neg_cropped_img)

    #     neg_db_line_12 = np.zeros((param.img_size_12, param.img_size_12, param.input_channel), np.float32)
    #     neg_db_line_24 = np.zeros((param.img_size_24, param.img_size_24, param.input_channel), np.float32)
    #     neg_db_line_48 = np.zeros((param.img_size_48, param.img_size_48, param.input_channel), np.float32)

    #     neg_cropped_arr_12 = util.img2array(neg_cropped_img, param.img_size_12)
    #     neg_cropped_arr_24 = util.img2array(neg_cropped_img, param.img_size_24)
    #     neg_cropped_arr_48 = util.img2array(neg_cropped_img, param.img_size_48)

    #     neg_db_line_12[:] = neg_cropped_arr_12
    #     neg_db_line_24[:] = neg_cropped_arr_24
    #     neg_db_line_48[:] = neg_cropped_arr_48

    #     neg_db_12.append(neg_db_line_12)
    #     neg_db_24.append(neg_db_line_24)
    #     neg_db_48.append(neg_db_line_48)

    # neg_db_12 = np.array(neg_db_12)
    # neg_db_24 = np.array(neg_db_24)
    # neg_db_48 = np.array(neg_db_48)
